...
static auto firstLineOffsetForBaselineAdjustment(const TextFrameLine& firstLine,
        ...
        -> Pair<STUFirstLineOffsetType, stu::Float64>
{
...
static inline stu::Float32 heightBelowBaselineWithoutExcessSpacing(const TextFrameLine& line) {
  return min(line._heightBelowBaseline,
             line._heightBelowBaselineWithoutSpacing + line.leading/2);
}

/// Does no displayScale rounding.
static stu::Float64 heightWithMinimalSpacingBelowLastBaseline(const TextFrameLayouter& layouter) {
...
void TextFrameLayouter::layoutAndScale(Size<stu::Float64> frameSize,
                                       ...
                                       const TextFrameOptions& options)
{
  ...
  struct State {
    ...
    stu::Float64 maxInverselyScaledHeight;
    CGFloat lowerBound;
    CGFloat upperBound;
  ...
  const stu::Int32 maxLineCount =    options.maximumNumberOfLines > 0
                             && options.maximumNumberOfLines <= maxValue<stu::Int32>
                           ? narrow_cast<stu::Int32>(options.maximumNumberOfLines) : maxValue<stu::Int32>;
  const stu::Float64 unlimitedHeight = 1 << 30;
  ...
  layout(Size{frameSize.width,
         ...
         state.scaleInfo,
         shouldEstimateScaleFactor ? maxValue<stu::Int32> : maxLineCount,
         options);
  ...
  const auto updateScaleInfo = [&](CGFloat scale) {
    ...
    const stu::Float64 inverseScale = 1.0/scale;
  ...
  const auto updateScaleInfoAndLayout = [&](CGFloat scale) {
    ...
    stu::Float64 height = unlimitedHeight;
    stu::Int32 lineCount = maxValue<stu::Int32>;
  ...
  const auto fits = [&]{
    ...
    const stu::Float64 height = heightWithMinimalSpacingBelowLastBaseline(*this);
...
static
stu::Float64 computeWidth(CTTypesetter* typesetter, Range<stu::Int32> stringRange, stu::Float64 headIndent) {
...
STU_NO_INLINE
stu::Float64 TextFrameLayouter::estimateTailTruncationTokenWidth(const TextFrameLine& line,
...
struct ScalingPara {
  stu::Int32 minLineCount{1};
  stu::Int32 maxLineCount;
  stu::Int32 lineCount;
  const stu::Int32 originalLineCount;
  const stu::Int32 initialLinesCount;
  stu::Float64 singleLineInverseScale{0};
  const stu::Float64 lineHeight;
  const Range<stu::Int32> stringRange;
  const stu::Float64 commonHeadIndent;
  const CGFloat initialExtraHeadIndent;
  const CGFloat initialExtraTailIndent;
  const stu::Float64 maxWidthMinusCommonIndent;
  ...
  /// for hyphenation).
  void bisectInverseScaleInterval(bool lineCountIsLowerBound, stu::Float64 inverseScale,
                                  CTTypesetter* const typesetter, const NSStringRef& string)
  {
    ...
    if (maxLineCount == 2 && commonHeadIndent == 0) {
      if (singleLineInverseScale == 0) {
        const stu::Float64 initialHeadIndent = max(0.f, initialExtraHeadIndent);
        const stu::Float64 initialTailIndent = max(0.f, initialExtraTailIndent);
        const stu::Float64 w = computeWidth(typesetter, stringRange,
    ...
    stu::Float64 initialHeadIndent = commonHeadIndent*inverseScale;
    stu::Float64 nonInitialHeadIndent = initialHeadIndent;
    stu::Float64 initialMaxWidth = maxWidthMinusCommonIndent*inverseScale;
    stu::Float64 nonInitialMaxWidth = initialMaxWidth;
    if (initialExtraHeadIndent != 0) {
      const stu::Float64 extraHeadIndent = initialExtraHeadIndent;
      if (initialExtraHeadIndent > 0) {
    ...
    if (initialExtraTailIndent != 0) {
      const stu::Float64 extraTailIndent = initialExtraTailIndent;
      if (initialExtraTailIndent > 0) {
    ...
    for (stu::Int32 n = 1, index = stringRange.start, endIndex;; ++n, index = endIndex) {
      const stu::Float64 maxWidth = n <= initialLinesCount ? initialMaxWidth : nonInitialMaxWidth;
      const stu::Float64 headIndent = n <= initialLinesCount ? initialHeadIndent : nonInitialHeadIndent;
      endIndex = index + narrow_cast<stu::Int32>(CTTypesetterSuggestLineBreakWithOffset(
                                              typesetter, index, maxWidth, headIndent));
      if (STU_UNLIKELY(endIndex <= index)) {
        endIndex = narrow_cast<stu::Int32>(string.endIndexOfGraphemeClusterAt(index));
      }
...
auto TextFrameLayouter::estimateScaleFactorNeededToFit(stu::Float64 frameHeight, stu::Int32 maxLineCount,
                                                       NSAttributedString* __unsafe_unretained
                                                         truncationToken,
...
-> ScaleFactorEstimate
{
  ...
  if (STU_UNLIKELY(mayExceedMaxWidth_)) {
    ...
    const stu::Float64 inverseScale = scaleInfo_.inverseScale;
    ...
    for (auto& para : paras_) {
      ...
      stu::Float64 initialExtraIndent = 0;
      stu::Float64 nonInitialExtraIndent = 0;
      stu::Float64 maxWidth = frameWidth;
      ...
      do {
        const stu::Float64 width = (lineIndex < para.initialLinesEndIndex
  ...
  stu::Float64 height = heightWithMinimalSpacingBelowLastBaseline(*this);
  if (scale*height <= frameHeight && lines.count() <= maxLineCount) {
    return {scale, true};
  }
  STU_ASSERT(maxLineCount > 0);

  const stu::Float64 firstLineOffset = scaleInfo_.scale == 1
  ...
  stu::Int32 lineCount = narrow_cast<stu::Int32>(lines_.count());
  if (lineCount > maxLineCount) {
    // Note that a paragraph may contain multiple forced line breaks (e.g. U+2028).
    stu::Int32 newlineCount = 0;
    stu::Int linesEndIndex = lines.count();
    for (auto& line : lines) {
    ...
    if (newlineCount >= maxLineCount) {
      stu::Float64 lastLineExtraWidth = 0;
      ...
      for (stu::Int i0 = 0, i = 0; i < lines.count(); i0 = i) {
        do {
        ...
        stu::Float64 headIndent = 0;
        stu::Float64 extraIndent = 0;
        stu::Float64 maxWidth = inverselyScaledFrameSize_.width;
        if (STU_UNLIKELY(p.isIndented)) {
          const stu::Float64 commonLeftIndent = p.commonLeftIndent*scaleInfo_.inverseScale;
          const stu::Float64 commonRightIndent = p.commonRightIndent*scaleInfo_.inverseScale;
          maxWidth -= commonLeftIndent + commonRightIndent;
          stu::Float64 extraIndentLeft;
          stu::Float64 extraIndentRight;
          if (isInitialLine) {
        ...
        const Range<stu::Int32> range{firstLine.rangeInOriginalString.start,
                                 lastLine.rangeInOriginalString.end};
        const stu::Float64 width = extraIndent
        ...
        const stu::Float32 lastLineHeightBelowBaseline =
                        !lastLine.isLastLine ? lastLine._heightBelowBaseline
                        : heightBelowBaselineWithoutExcessSpacing(lastLine);
        stu::Float32 maxHeightAboveBaseline = 0;
        stu::Float32 maxHeightBelowBaseline = 0;
        ...
        const stu::Float64 d = (lastLine.originY - firstLine.originY)
  ...
  for (stu::Int32 i0 = 0, i = 0; i < lines.count(); i0 = i) {
    ...
    const stu::Int32 initialLinesEndIndex = paras_[firstLine.paragraphIndex].initialLinesEndIndex;
    const ShapedString::Paragraph& p = stringParas()[firstLine.paragraphIndex];
    multiLineParaHasHyphenation |= p.hyphenationFactor > 0;
    stu::Float64 commonHeadIndent = 0;
    CGFloat initialExtraHeadIndent = 0;
    CGFloat initialExtraTailIndent = 0;
    stu::Float64 maxWidthMinusCommonIndent = inverselyScaledFrameSize_.width;
    if (STU_UNLIKELY(p.isIndented)) {
      const stu::Float64 commonLeftIndent = p.commonLeftIndent*scaleInfo_.inverseScale;
      const stu::Float64 commonRightIndent = p.commonRightIndent*scaleInfo_.inverseScale;
  ...
  TempVector<stu::Int32> remainingParaIndices{Capacity{paras.count()}, paras.allocator()};
  remainingParaIndices.append(repeat(uninitialized, paras.count()));
  for (stu::Int i = 0; i < paras.count(); ++i) {
    remainingParaIndices[i] = static_cast<stu::Int32>(i);
  }
  stu::Float64 lowerBound = minScale;
  stu::Float64 upperBound = scale;
  bool isLowerBound = false;
  while (lowerBound + accuracy < upperBound) {
    scale = (lowerBound + upperBound)/2;
    const stu::Float64 inverseScale = 1/scale;
    stu::Float64 savedHeight = 0;
    stu::Int32 savedLineCount = 0;
    remainingParaIndices.removeWhere([&](stu::Int32 i) -> bool {
      ScalingPara& para = paras[i];
      para.bisectInverseScaleInterval(isLowerBound, inverseScale,
                                      typesetter_, attributedString_.string);
      const stu::Int32 lineCountDiff = para.originalLineCount - para.lineCount;
      const stu::Float64 heighDiff = lineCountDiff*para.lineHeight;
    ...
    const stu::Int32 newLineCount = lineCount - savedLineCount;
    const stu::Float64 newHeight = height - savedHeight;
...
auto TextFrameLayouter::calculateMaxScaleFactorForCurrentLineBreaks(stu::Float64 maxHeight) const
  -> ScaleFactorAndNeedsRealignment
{
  if (STU_UNLIKELY(lines_.isEmpty())) return {1, false};
  const stu::Float64 height = heightWithMinimalSpacingBelowLastBaseline(*this);
  stu::Float64 scale = height <= 0 ? 1 : maxHeight/height;
  ...
  for (const TextFrameParagraph& para : paras_) {
    ...
    {
      stu::Float32 width32 = lines_[i].width;
