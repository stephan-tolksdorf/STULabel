...
private:
  ...
  UInt  bufferSize_{};
  UInt  index_{};
  Vector<Pair<stu::Byte*, UInt>, 1, AllocatorRef> previousBuffers_;
  ...
  STU_INLINE
  void deallocateImpl(stu::Byte* pointer, UInt minSize) noexcept {
  #if STU_USE_ADDRESS_SANITIZER
    if (minSize > 0
  ...
  STU_INLINE __attribute__((alloc_size(1 + 4)))
  stu::Byte* decreaseCapacityImpl(stu::Byte* pointer, UInt usedSize __unused, UInt oldSize, UInt newSize)
          noexcept
  {
    const uintptr_t index = reinterpret_cast<uintptr_t>(pointer)
                          - reinterpret_cast<uintptr_t>(buffer_);
  ...
  STU_NO_INLINE
  stu::Byte* allocate_slowPath(UInt size) {
    const UInt roundedUpSize = roundUpToMultipleOf<minAlignment>(size + minAllocationGap);
