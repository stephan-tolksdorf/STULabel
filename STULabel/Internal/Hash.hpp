...
HashCode<stu::UInt64> hash(stu::UInt64 low, stu::UInt64 high) {
  const stu::UInt64 f = 0x9ddfea08eb382d69;
  stu::UInt64 a = (low ^ high)*f;
  a ^= (a >> 47);
  uint64_t b = (high ^ a)*f;
  b ^= (b >> 47);
  b *= f;
...
[[nodiscard]] STU_CONSTEXPR
HashCode<stu::UInt64> hash(stu::UInt64 value) {
  return hash(value & maxValue<stu::UInt32>, value >> 32);
}

template <typename T>
[[nodiscard]] STU_CONSTEXPR
HashCode<stu::UInt64> hashPointer(T* pointer) {
  static_assert(sizeof(pointer) <= 8);
  return hash(reinterpret_cast<stu::UInt64>(pointer));
}
...
void hashableBits(Sink sink, T value) {
  ...
  sink(static_cast<stu::UInt64>(value));
}

template <typename T, EnableIf<isOneOf<T, Float32, Float64>> = 0>
STU_CONSTEXPR
Conditional<sizeof(T) == 4, stu::UInt32, stu::UInt64> hashableBits(T value) {
  using U = Conditional<sizeof(T) == 4, stu::UInt32, stu::UInt64>;
...
namespace detail {
  ...
  STU_CONSTEXPR
  stu::UInt64 hashPairwise(const stu::UInt64 (& array)[n], Indices<indices...>) {
    static_assert(n/2 == sizeof...(indices));
    if constexpr (n == 2) {
      return hash(array[0], array[1]).value;
    } else if constexpr (sizeof...(indices)*2 == n) {
      const stu::UInt64 reduced[] = {hash(array[2*indices], array[2*indices + 1]).value...};
      return hashPairwise(reduced, MakeIndices<n/4>{});
    } else {
      const stu::UInt64 reduced[] = {hash(array[2*indices], array[2*indices + 1]).value..., array[n - 1]};
...
[[nodiscard]] STU_CONSTEXPR
HashCode<stu::UInt64> hash(const A& a, const Ts&... args) {
  stu::UInt64 result = 0;
