...
- (void)testHash {
  static_assert(hash(0u, 0u) == 0);
  static_assert(hash(0xffffffffffffffff, 0xffffffffffffffff) == 0xe286fb3ae0b4292f);
  static_assert(hash(0xfedcba9876543210u, 0x7654321089abcdefu) == 0x3d9dc4e3bde6427d);
...
#else
  XCTAssertEqual(hash(0, 0), 0);
  XCTAssertEqual(hash(0xffffffffffffffff, 0xffffffffffffffff), 0xe286fb3ae0b4292f);
  XCTAssertEqual(hash(0xfedcba9876543210, 0x7654321089abcdef), 0x3d9dc4e3bde6427d);
  XCTAssertEqual(hash(0xfedcba9876543210), hash(0x76543210, 0xfedcba98));
...
#endif
  XCTAssertEqual(hash(0.f), 0u);
  XCTAssertEqual(hash(-0.f), 0u);
  XCTAssertEqual(hash(0.0), 0u);
  XCTAssertEqual(hash(-0.0), 0u);
