...
TEST(ZeroBuffer) {
  ...
  CHECK_EQ(alloc.freeCapacityInCurrentBuffer<stu::Byte>(), 0);
  stu::Byte* const p0 = alloc.allocate(1);
  p0[0] = 0;
  CHECK(p0 != (stu::Byte*)(&buffer));
  alloc.deallocate(p0);
  const Int minAllocationGap = ArenaAllocator<>::minAllocationGap;
  CHECK_EQ(alloc.freeCapacityInCurrentBuffer<stu::Byte>(), 4096 - minAllocationGap);
}
...
TEST(AllocateDeallocate) {
  ...
  CHECK_EQ((stu::Byte*)p2, p0 + offset2);
  p2[0] = 2;
  p2[n - 1] = 2;
  CHECK_EQ(alloc.freeCapacityInCurrentBuffer<stu::Byte>(), 0);
  ...
  stu::Byte* const p4 = alloc.allocate(bufferSize - minAllocationGap);
  CHECK_EQ(p0, p4);
  alloc.deallocate(p4, bufferSize - minAllocationGap);
  stu::Byte* const p5 = alloc.allocate(1);
  CHECK_EQ(p0, p5);
  stu::Byte* const p6 = alloc.allocate(bufferSize);
  CHECK(p6 != (stu::Byte*)p1);
  alloc.deallocate(p6, bufferSize);
  const Int n2 = alloc.freeCapacityInCurrentBuffer<stu::Byte>() + 1;
  CHECK_EQ(n2, 4096 - minAllocationGap + 1);
  stu::Byte* const p7 = alloc.allocate(n2);
  CHECK(p7 != (stu::Byte*)p6);
  p7[0] = 7;
  p7[n2 - 1] = 7;
  ...
  stu::Byte* const p8 = alloc.allocate(n3);
  p8[0] = 8;
  p8[n3 - 1] = 8;
  alloc.deallocate(p8, n3);
  CHECK_EQ(alloc.freeCapacityInCurrentBuffer<stu::Byte>(), n3 + 1);
...
TEST(IncreaseDecreaseCapacity) {
  ...
  ArenaAllocator<>::InitialBuffer<bufferSize> buffer;
  ArenaAllocator<> alloc{Ref{buffer}};
  stu::Byte* const p0 = alloc.allocate(4);
  CHECK_EQ(alloc.increaseCapacity(p0, 4, 4, 6), p0);
  p0[4] = 0;
  p0[5] = 0;
  CHECK_EQ(alloc.increaseCapacity(p0, 6, 6, 8), p0);
  const int n = 8 + (int)alloc.freeCapacityInCurrentBuffer<stu::Byte>();
  ...
  p0[0] = 1;
  stu::Byte* const p1 = alloc.allocate(1);
  CHECK(p0 < p1);
  stu::Byte* const p2 = alloc.increaseCapacity(p0, 1, 1, 2);
  CHECK(p1 < p2);
  CHECK_EQ(alloc.decreaseCapacity(p1, 0, 1, 0), p1);
  const int n2 = (int)alloc.freeCapacityInCurrentBuffer<stu::Byte>() + 1;
  stu::Byte* const p4 = alloc.increaseCapacity(p1, 0, 0, n2);
  CHECK(p1 != p4);
  p4[0] = 4;
  p4[n2 - 1] = 4;
